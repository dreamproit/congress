...
def parse_nomination(nomination_id, body, options):
    ...
    # the markup on these pages is a disaster, so we're going to use a heuristic based on boldface, inline tags followed by text # noqa
        ...
        if pair.tail:
            ...
            if label.split(" ")[-1] == "Action":
                ...
                if label == "Organization":
                    ...
                    match = re.search(r"(\d{2,3})[stndhr]{2}", data)
                    if match:
