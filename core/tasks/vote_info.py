    ...
    if str(dom.xpath("string(document/document_type)")):
        if dom.xpath("string(document/document_type)") == "PN":
            ...
            vote["bill"] = {
                ...
                ),  # some historical files don't have document/document_congress so take the first of document/document_congress or the top-level congress element as a fall-back # noqa
...
def normalize_vote_type(vote_type):
    ...
    mapping = (
        ...
        (
            r"(On Motion to )?(Concur in|Concurring|On Concurring|Agree to|On Agreeing to) (the )?Senate (Amendment|amdt|Adt)s?",  # noqa
        ...
        (
            r"On the Motion \(Motion to ((Recede )from the Senate Amendment to \S+ \d+ (and ))?Concur( with Further Amendment)?",  # noqa
...
def get_vote_category(vote_question):
    ...
    mapping = (
        ...
        # house only
        (
            r"^(On Motion (to|that the House) )?(Concur in|Concurring|Concurring in|On Concurring|On Concurring in|Agree to|On Agreeing to) (the )?Senate (Amendment|amdt|Adt)s?",  # noqa
            "passage",
        ),
        (
