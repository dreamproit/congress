...
def sponsor_for(sponsor_dict):
    ...
    m = re.match(
        r'(?P<title>(Rep\.|Sen\.|Del\.|Resident Commissioner)) (?P<name>.*?) +\[(?P<party>[DRIL])-(?P<state>[A-Z][A-Z])(-(?P<district>\d{1,2}|At Large|None))?\]$',  # noqa
    ...
    result = {
        'title': title,
        'name': name,  # the firstName, middleName, lastName fields have inconsistent capitalization - some are all uppercase # noqa
...
def strip_tags(text):
    # Preserve paragraph breaks. Convert closing p tags (and surrounding whitespace) into two newlines. Strip trailing whitespace # noqa
...
def current_title_for(titles, title_type):
    ...
    for title in titles:
        if title['type'] != title_type or title['is_for_portion'] is True:
            continue
...
def action_for(item):
    ...
    # sometimes there are links (one case is for bills passed by a rule in a resolution, the link will point to the resolution) # noqa
        ...
        cosponsor_dict.update(
            {
                ...
                # 'withdrawn_at': item['sponsorshipWithdrawnDate'], # no longer present in GPO BILLSTATUS XML schema 3.0.0? # noqa
...
def parse_bill_action(action_dict, prev_status, bill_id, title):
    ...
    m = re.search(
        ...
        + "|".join(
            [
                ...
                "On motion (?:that )?the House (?:suspend the rules and )?(?:agree(?: with an amendment)? to|concur in) the Senate amendments?(?: to the House amendments?| to the Senate amendments?)*",  # noqa
            ]
        )
        + ")"
    ...
    m = re.search(
        r"Passed House pursuant to|House agreed to Senate amendment (with amendment )?pursuant to|Pursuant to the provisions of [HSCONJRES\. ]+ \d+, [HSCONJRES\. ]+ \d+ is considered passed House",  # noqa
        line,
        re.I,
    )
    if m is not None:
    ...
    # House motions to table adversely dispose of a pending matter, if agreed to. An agreed-to "motion to table the measure", # noqa
    ...
    if m is not None:
        ...
        # In order to classify this as resulting in the same thing as regular failed vote on passage, new_status_after_vote # noqa
    ...
    m = re.search(
        ...
        + "|".join(
            [
                ...
                "Senate (?:agreed to|concurred in) (?:the )?(?:conference report|House amendment(?: to the Senate amendments?| to the House amendments?)*)",  # noqa
                ...
                "Cloture on (?:the motion to (?:proceed to |concur in )(?:the House amendment (?:to the Senate amendment )?to )?)(?:the bill|H.R. .*) (?:not )?invoked in Senate",  # noqa
                "(?:Introduced|Received|Submitted) in the Senate, (?:read twice, |considered, |read the third time, )+and (?:passed|agreed to)",  # noqa
            ]
        )
        + ")"
        + "(,?.*,?) "
        + r"(without objection|by Unanimous Consent|by Voice Vote|(?:by )?Yea-Nay( Vote)?\. \d+\s*-\s*\d+\. Record Vote (No|Number): \d+)",  # noqa
        line.replace("  ", " "),
        re.I,
    )
    if m is not None:
    ...
    m = re.search(
        r"Measure passed (House|Senate)(, amended(?: \(.*?\)|, with an amendment to the title)?)?(?:,? in lieu[^,]*)?(?:, roll call #(\d+) \(\d+-\d+\))?",  # noqa
        line,
        re.I,
    )
    if m is not None:
        ...
        roll_num = m.group(3)
        # GovTrack legacy scraper missed these: if chamber == 's' and (as_amended or roll_num or "lieu" in line): return action, status # noqa
        pass_fail = "pass"
    ...
    m = re.search(
        r"(House|Senate) agreed to (?:House|Senate) amendments?( with an amendment)?( under Suspension of the Rules)?(?:, roll call #(\d+) \(\d+-\d+\))?\.",  # noqa
        line,
        re.I,
    )
    if m is not None:
        ...
        roll_num = m.group(4)
        # GovTrack legacy scraper missed these: if (chamber == 'h' and not roll_num) or (chamber == 's' and rull_num): return action, status # noqa
        pass_fail = "pass"
        vote_type = "pingpong"
        action["type"] = "vote"
        action["vote_type"] = vote_type
    ...
    # Ordered Reported (because GovTrack did this, but maybe should be changed to not combine with actual reported bills) # noqa
    m = re.search(
    ...
    if m is not None:
        ...
        if prev_status in (
            ...
            raise Exception(
                "Missing Signed by President action? If this is a case of the 10-day rule, hard code the bill id %s here."  # noqa
                % bill_id
            )
