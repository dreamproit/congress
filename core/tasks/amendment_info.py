...
from core.tasks.bill_info import action_for
from core.tasks.bill_info import sponsor_for as sponsor_for_bill
...
def create_govtrack_xml(amdt, options):
    ...
    make_node(
        root,
        "description",
        amdt.get("description") if amdt.get("description") else amdt["purpose"],
    )
...
def parse_amendment_actions(actions):
    for action in actions:
        # House Vote
        m = re.match(
            r"On agreeing to the .* amendments? (\(.*\) )?(?:as (?:modified|amended) )?(Agreed to|Failed) (without objection|by [^\.:]+|by (?:recorded vote|the Yeas and Nays): (\d+) - (\d+)(, \d+ Present)? \(Roll [nN]o. (\d+)\))\.",  # noqa
            action['text'],
        )
        if m:
            action["where"] = "h"
        ...
        # Senate Vote
        m = re.match(
            r"(Motion to table )?Amendment SA \d+(?:, .*?)? (as modified )?(agreed to|not agreed to) in Senate by ([^\.:\-]+|Yea-Nay( Vote)?. (\d+) - (\d+)(, \d+ Present)?. Record Vote Number: (\d+))\.",  # noqa
            action['text'],
        )
        if m:
            action["type"] = "vote"
